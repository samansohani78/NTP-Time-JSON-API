...
> **Why not financial-ready yet.** The upstream client (`rsntp`) discards the raw timestamps and
> root metrics needed for honest accuracy; the UDP server advertises `root_dispersion = 0`; there is
> no time-quality envelope or serve/stop SLA; no secure manual-override API; and no real E2E test
> harness. Until P0 lands, the `ntp_timing` T2/T3 values in `/performance` are algebraic
> reconstructions of offset/delay (honest for offset/delay, but **not** independent server
> measurements). See **`PRODUCTION_ACCURACY_PLAN.md`** (P0/P1/P2, phased) for the implementation-ready plan.
...
|------|-----|--------|----------------|---------------------|--------------------|
| **P0-1** Packet-level async NTP client (real T1-T4) | P0 | todo | `src/ntp/client.rs`*(new)*, `src/ntp/protocol.rs`, `src/ntp/mod.rs` | Client returns **measured** T2/T3 byte-identical to server reply; origin-mismatch / KoD / leap-alarm / zero-transmit / negative-delay / timeout all rejected; no `rsntp` in client | `cargo test ntp::client` |
| **P0-2** Carry real fields end-to-end | P0 | todo | `src/ntp/sync.rs`, `src/ntp/selection.rs`, `src/http/state.rs`, `src/main.rs`, `src/timebase.rs` | `/performance` shows `timing_source:"measured"` + real `root_delay_ms`/`root_dispersion_ms`/`stratum`/`leap`; `last_sync_quality` populated each sync | `cargo test ntp:: && cargo run` + `curl /performance` |
| **P0-3** Honest UDP `root_delay`/`root_dispersion` | P0 | todo | `src/ntp/server.rs`, `src/main.rs`, `src/metrics.rs` | Synced UDP replies carry `root_dispersion>0` growing with sync age, bounded by `MAX_ROOT_DISPERSION_MS`; `root_delay>=upstream_root_delay` | `cargo test ntp::server` + `tests/e2e_ntp_udp.rs` |
| **P0-4** Time-quality envelope + `/status` + serve/stop SLA | P0 | todo | `src/http/handlers.rs`, `src/http/mod.rs`, `src/performance.rs`, `src/http/websocket.rs`, `src/config.rs`, `src/metrics.rs` | `/status` full envelope; `/time` byte-compatible + quality headers; 503 at hard limits; `source` never silently wrong | `cargo test http:: ` + `curl /status` |
| **P0-5** Real integration / E2E harness + CI | P0 | todo | `src/lib.rs`*(new)*, `src/main.rs`, `tests/common/mod.rs`*(new)*, `tests/e2e_*.rs`, `.github/workflows/ci.yml`, `Makefile` | placeholder `integration_api.rs` gone; HTTP+UDP+WS+metrics exercised vs running server; CI `e2e` job green | `make e2e` |
| **P1-6** Uncertainty-scored selection (weighted-median + quorum) | P1 | todo | `src/ntp/selection.rs`, `src/ntp/sync.rs`, `src/ntp/stats.rs`, `src/config.rs`, `src/metrics.rs` | 11 adversarial cases green; min-RTT fallback removed; `/status` exposes `quorum_size`/`combined_uncertainty_ms`/`rejected_sources` | `cargo test ntp::selection` |
| **P1-7** Secure manual time-override API **(core requirement — P0/P1 boundary, "P1-high")** | P1-high | todo *(needs security review)* | `src/timebase.rs`, `src/http/handlers_admin.rs`*(new)*, `src/http/mod.rs`, `src/http/middleware.rs`, `src/http/state.rs`, `src/ntp/server.rs`, `src/config.rs`, `src/metrics.rs` | disabled-by-default→404; auth enforced; set/clear/TTL/max-jump/force all tested; `source:"manual"` + UDP `MANU` everywhere; monotonic preserved; audit logged | `cargo test --test e2e_manual_override` |
| **P1-8** Replica-drift visibility (no consensus) | P1 | todo | `src/http/handlers.rs`, `src/config.rs`, `src/metrics.rs`, `k8s/prometheus-rules.yaml`*(new)*, `PROJECT_ARCHITECTURE.md` | `/status` shows `replica_id`+uncertainty; `/readyz`→503 above `READINESS_MAX_UNCERTAINTY_MS`; inter-replica spread alertable | `cargo test http::` + Prom rule lint |
| **P1F-12** Marzullo / interval-intersection (or NTP clock-combine) selection — robustness follow-up to P1-6 | P1-followup | todo | `src/ntp/selection.rs`, `src/ntp/sync.rs`, `src/config.rs`, `src/metrics.rs` | Adversarial **independently-wrong majority** test must **fail closed** (no confident wrong pick): either reject via interval-intersection, apply source trust tiers, or expose uncertainty high enough that the P1-8 readiness gate drops the replica. No silent wrong selection. | `cargo test ntp::selection` |
...
> **Human sign-off required** (see `PRODUCTION_ACCURACY_PLAN.md` §9): D1 manual-mode LI behavior, D2 hard-stop default, D6 admin auth/secrets, D8 SLA numbers, P1-7 security review, OPS-2 history rewrite.
